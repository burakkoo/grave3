...
export default {
  ...
  callbacks: {
    ...
    authorized({ auth, request: { nextUrl } }) {
      ...
      const unProtectedPages = ['/terms', '/privacy-policy', '/profile', '/memoria-page', '/discover']; // Add more here if needed
      ...
      if (isOnAuthPage) {
        ...
        if (isLoggedIn) return NextResponse.redirect(new URL(`/discover`, nextUrl));
