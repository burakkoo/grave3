...
export async function serverWritePost({ formData, type, postId }: Props) {
  ...
  try {
    ...
    const { content, files, profileId, name, relation } = body;
    ...
    // Auto-approve if the poster is the profile owner
    const isAutoApproved = user?.id === profileId;
    ...
    if (type === 'create') {
      const res = await prisma.post.create({
        data: {
          ...
          PostedBy: name,
          Relation: relation,
          ApprovalStatus: isAutoApproved, // Posts require approval unless made by profile owner
        },
        select: selectPost(user?.id),
      });

      // Log mentions activity only if user is authenticated
      if (user) {
