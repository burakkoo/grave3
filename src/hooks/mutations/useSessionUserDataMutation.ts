...
export function useSessionUserDataMutation() {
  ...
  // Mutation for saving positionY of cover photo
  const updatePositionYMutation = useMutation({
    mutationFn: async ({ userId, positionY }: { userId: string; positionY: number }) => {
      const res = await fetch(`/api/users/${userId}/cover-photo-position`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!res.ok) {
        throw new Error('Error saving cover photo position.');
      }
  ...
  return {
    ...
    updateSessionUserPhotosMutation,
    updatePositionYMutation, // Return the positionY mutation
  };
}
